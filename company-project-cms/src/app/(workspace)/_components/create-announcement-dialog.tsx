...
export function CreateAnnouncementDialog({
  ...
  const onSubmit = async (data: z.infer<typeof announcementSchema>) => {
    try {
      ...
      // Create FormData for handling both text fields and files
      const formData = new FormData();
      
      // Add text fields directly to FormData (not as JSON)
      formData.append('title', data.title.trim());
      formData.append('content', data.content.trim());
      
      // Handle mentions - ensure it's a well-formatted JSON string
      if (data.mentions && data.mentions.length > 0) {
        const mentionString = JSON.stringify(data.mentions);
        console.log('Mentions as JSON string:', mentionString);
        formData.append('mention', mentionString);
      }
      ...
      // Append each file to the FormData if any
      if (files.length > 0) {
      ...
      if (axiosError.code === 'ECONNABORTED') {
        ...
        const errorMessage =
          axiosError.response?.data?.message ?? "Failed to create post. Please try again.";
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px] p-0 dark:bg-slate-950 bg-slate-100 text-slate-950 dark:text-slate-200 h-[80vh] flex flex-col overflow-hidden">
