...
import { db } from "../db";
import { userTable, companyTable } from "../db/schema";
import bcrypt from "bcryptjs";
import { v4 as uuidv4 } from "uuid";
import { eq, ilike, or, and, sql, SQL, count } from "drizzle-orm";
  ...
  try {
    ...
    if (!companyId || req.user.role !== "admin") {
      return res.status(401).json(
        new ApiResponse(401, {}, "Unauthorized: You are not an admin or company not found")
      );
    }
    ...
    const filter = req.query.filter ? (req.query.filter as string) : undefined;
    ...
    if (search) {
      conditions.push(
        ilike(sql`${userTable.firstName} || ' ' || ${userTable.lastName}`, `%${search}%`)
      );
    }
